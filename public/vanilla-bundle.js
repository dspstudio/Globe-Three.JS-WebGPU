...
var use2k = typeof window !== "undefined" && (isMobileOrTablet() || getInitialResolutionScale() < 1);
var CONSTANTS = {
  RENDER_TYPE: "webgpu",
  // 'webgpu' or 'webgl'
  EARTH_RADIUS: 10,
  ...
  TEXTURES: {
    ...
    SPECULAR: use2k ? "./2k_earth_specular_map.jpg" : "./8k_earth_specular_map.jpg",
...
function buildGui(gui, options) {
  ...
  skyFolder.add(scene.backgroundRotation, "y", 0, Math.PI * 2).step(0.01).name("Yaw / Longitude (Y)");
  skyFolder.add(scene.backgroundRotation, "x", 0, Math.PI * 2).step(0.01).name("Pitch / Latitude (X)");
  skyFolder.add(scene.backgroundRotation, "z", 0, Math.PI * 2).step(0.01).name("Roll (Z)");
...
var Engine = class {
  ...
  async init(onProgress) {
    ...
    this.scene.backgroundIntensity = CONSTANTS.GUI.ENVIRONMENT.SKYBOX_INTENSITY;
    this.scene.backgroundRotation.y = CONSTANTS.GUI.ENVIRONMENT.SKYBOX_AZIMUTH;
    ...
    const flareColorModified = this.sunColorUniform.mul(vec33(1.2, 1.2, 1.2)).mul(lF);
